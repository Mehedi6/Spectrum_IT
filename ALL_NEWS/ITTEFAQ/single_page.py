...
class NewsScraper:
    ...
    def scrape_news_data(self, url, news_type, sub_category):
        ...
        try:
            ...
            news_data['meta_description'] = meta_description['content'] if meta_description else None
        except Exception as e:
            print(f"Error {e}")
            news_data['meta_description'] = None

        base_url = "https://www.ittefaq.com.bd"  # Replace with the website's base URL
        try:
            chrome_options = Options()
            chrome_options.add_argument("--headless=new")
            driver = webdriver.Chrome(options=chrome_options)
            ...
            # Get all img tags
            images = soup1.find('img',{'id': 'adf-overlay'})
        ...
        try:
            # Content
            content_elements = soup.select('#widget_322 > div > div > div > div.content_detail_outer > div.content_detail_inner > div.content_detail_left > div > div > div.row.detail_holder > div > div > div.detail_inner > div.content_detail_content_outer > div > article > div > p')
        ...
        # Author
        try:
            author = soup.select_one('#widget_322 > div > div > div > div.content_detail_outer > div.content_detail_inner > div.content_detail_left > div > div > div.row.detail_holder > div > div > div.additional_info_container > div > div.each_row.author_n_share > div > span')
        ...
        try:
            ...
            published_date_element = soup.select_one('#widget_322 > div > div > div > div.content_detail_outer > div.content_detail_inner > div.content_detail_left > div > div > div.row.detail_holder > div > div > div.additional_info_container > div > div.each_row.time > span.tts_time')
        ...
        # Updated Date
        try:
            updated_date_element = soup.select_one('#widget_322 > div > div > div > div.content_detail_outer > div.content_detail_inner > div.content_detail_left > div > div > div.row.detail_holder > div > div > div.additional_info_container > div > div.each_row.time > span.dn'
            )
        ...
        # Additional Metadata
        news_data['source'] = 'দৈনিক ইত্তেফাক'
...
# Main Script
try:
    with open("C:\\Users\\arwen\\Desktop\\Newspaper Scraping\\Spectrum_IT\\ALL_NEWS\\ITTEFAQ\\ittefaq_data.json", 'r', encoding='utf-8') as file:
...
with open("C:\\Users\\arwen\\Desktop\\Newspaper Scraping\\Spectrum_IT\\ALL_NEWS\\ITTEFAQ\\sample_news_links.json", 'r', encoding='utf-8') as file:
...
with open("C:\\Users\\arwen\\Desktop\\Newspaper Scraping\\Spectrum_IT\\ALL_NEWS\\ITTEFAQ\\ittefaq_data.json", 'w', encoding='utf-8') as file:
